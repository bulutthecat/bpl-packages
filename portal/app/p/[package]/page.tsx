...
export default function Page({ params }: { params: { package: string } }) {
  ...
  return <div className="p-5 container mx-auto">
    <div className="border-default-300 border-b-1 pb-4 mb-4 flex justify-between items-center">
      <div>
        ...
        <p className="text-default-500">{params.package} • {info.version} • {info.author}</p>
      </div>
    ...
    <div className="space-y-4">
      {info.requires && <div>
        ...
        {info.requires?.map((p, i) => (
          <Link showAnchorIcon underline="hover" href={`/p/${p}`} key={i}>{p}</Link>
        ))}
      </div>}
      ...
      {info.homepage && <div>
        ...
        <Link isExternal showAnchorIcon underline="hover" href={info.homepage}>{info.homepage.split('://')[1]}</Link>
      </div>}
    </div>
  </div>;
}